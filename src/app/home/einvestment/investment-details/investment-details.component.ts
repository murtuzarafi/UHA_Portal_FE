...
export class InvestmentDetailsComponent implements OnInit {
  ...
  onBlurEvent(event: any) {
    ...
    if (reqAmount < minReInvestment) {
      ...
      this.filterFund.RequestedAmout = this.productAmount;
      this.filterFund.RequestedUNITS = 88
  ...
  isAllSelected(item) {
    ...
    ///////////////////////console.log('Selected Fund / Filter Fund', this.filterFund)
  ...
  SendRequest() {
    ...
    let DayID = new Date().toLocaleString('en-us', { weekday: 'long' })
    let PCT_CUTOFFTIME: string
    let PRD_PRODCODE = this.selectedProduct.FND_FUNDCODE;
  ...
  postData() {
    ...
    return new Promise<void>((resolve) => {
      ...
      {
          this.notify.showError("No product found in the basket", "Error")
          return 
      }
      ...
      this.data.push(this._IS_TRX_TRX_MASTER)
      this.data.push(this.trxRequested)
      this.data.push(this.lstpayment)
      ...
      //this.http.SaveTransations(this.data);
      resolve();
    });
  }
  ...
  SetTransactionMasterObject() {
    return new Promise<void>((resolve) => {
      ...
      this._IS_TRX_TRX_MASTER.TSM_FORMNO = "OEIT_" + this.UserData[0].ACC_Code + "_" + new Date().toLocaleString();
      ...
      this._IS_TRX_TRX_MASTER.TSM_TRXTYPE = "IT"
      this._IS_TRX_TRX_MASTER.CUS_CODE = null
      this._IS_TRX_TRX_MASTER.TSM_TRXNAME = "Online Investment"
  ...
  SetTransactionRequested(trxMaster: IS_TRX_TRX_MASTER) {
    ...
    return new Promise<void>((resolve) => {
      ...
      if (this.selectedProduct) {
        ...
        /*
         if (chkRebate.Checked)
         {
             trxRequested.TSD_REBATEPERCENT = 100;
         }
         else
         {
             trxRequested.TSD_REBATEPERCENT = 0;
         }
         */
        this.trxRequested.TSD_REBATEPERCENT = 100;
  ...
  */
      ...
      */
        ...
        trxPayment.PMT_TRANSACTIONTYPE = "IT";
        trxPayment.CLY_YEAR = 1;
        trxPayment.CLP_PERIOD = 1;
        trxPayment.PMT_INSTTYPE = this.Genid;
        trxPayment.PMT_INSTNAME = ""//GEN_NAME;
...
*/
  ...
  /*
    SetAccountPro(trxMaster) {

    let PRD_PRODUCTCODE: string = this.selectedProduct.FND_FUNDID;
    let PUC_CLASSCODE: string = this.selectedProduct.PUC_CLASSCODE;
    let PUT_TYPENAME: string = this.selectedProduct.PUT_TYPENAME;
    let FND_FUNDCODE: number = this.selectedProduct.FND_FUNDCODE;
    ...
    AccountProfile.PRP_PROFILEID = uuidv4();
    AccountProfile.PRD_PRODUCTID = PRD_PRODUCTCODE;
    AccountProfile.ACC_SYSTEMCODE = trxMaster.ACC_SYSTEMCODE;

    AccountProfile.ACC_CODE = trxMaster.TSM_ACCOUNTCODE;
    AccountProfile.FND_FUNDCODE = FND_FUNDCODE;
    AccountProfile.PUC_CLASSCODE = PUC_CLASSCODE;
    AccountProfile.PUT_UNITTYPE = PUT_TYPENAME;
    AccountProfile.PRP_FIXEDINCOME = 0;
    AccountProfile.PRP_ISZEROBALANCE = false;
    AccountProfile.PRP_STATUS = "Active";
    AccountProfile.PRP_PROFILECREATEDON = new Date;
    AccountProfile.PRP_CREATEDON = new Date;
    AccountProfile.PRP_CREATEDBY = "Online User";
    AccountProfile.PRP_DIGITALCERTIFICATE = "???";
    AccountProfile.PRP_VERSION = 1;
    AccountProfile.PRP_OPTIONMETHOD = "Both";
